...
async function getSculNm(school_name, msg) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        request.post(info, (err, response, body) => {
            ...
            if (data.schulCode == '') {
                msg.channel.send(`\`\`${data.schulNm}의 검색 결과가 없습니다. 정확한 이름을 입력해 주세요.\`\``);
                resolve(0);
            } else {
                msg.channel.send(`\`\`${data.schulNm}를 찾았습니다.\`\``);
...
async function Self_test(userinfo, msg) {
    
    getSculNm(userinfo.school.name, msg).then((schul) => {
        if (schul == 0) 
            return;
        ...
        request.post(reqData, (err, response, body) => {
            ...
            if (data.rtnRsltCode == "SUCCESS") {
                ...
                request.post(checkData, (err, response, body) => {
                    ...
                    if (data.rtnRsltCode == "SUCCESS")
                        return msg.channel.send(`\`\`${data.schulNm} ${data.stdntName} 학생의 자가진단을 완료했습니다.\`\``);
                    else 
                        return msg.channel.send(`\`\`잘못된 본인확인 정보입니다. (${data.rtnRsltCode})\n\r참여주소 또는 본인확인 정보를 확인바랍니다.\n\r확인 후 다시 시도해 주시기 바랍니다.\`\``);
                })
                
            } else {
                switch (data.rtnRsltCode) {
                    case "ADIT_CRTFC_NO":
                        msg.channel.send('``동일한 이름을 가진 학생이 있어, 웹 페이지에서 진행해 주시길 바랍니다.``'); break;
                    case "QSTN_USR_ERROR":
                        msg.channel.send('``잘못된 본인확인 정보(학교,성명,생년월일)를 입력하였습니다.\n\r확인 후 다시 시도해 주시기 바랍니다.``'); break;
                    case "SCHOR_RFLT_YMD_ERROR":
                        msg.channel.send('``학생 건강상태 자가진단 참여가능기간을 확인바랍니다.\n\r확인 후 다시 시도해 주시기 바랍니다.``'); break;
                    default:
                        msg.channel.send(`\`\`잘못된 본인확인 정보입니다.(${data.rtnRsltCode})\n\r확인 후 다시 시도해 주시기 바랍니다.\`\``); break;
                }
            }
        })
...
exports.Self_test = (user, msg) => { Self_test(user, msg) }